...
let buttonText;
  ...
  if(defaultValues) {
    ...
    [previewImageVal,image1Val, image2Val, image3Val, image4Val] = defaultValues.images;
    ...
    useEffect(() => {
        ...
        if(image1 && !isValidFileType(image1)) errors.image1 = "Image URL must end in .png, .jpg, or .jpeg" 
        if(image2 && !isValidFileType(image2)) errors.image2 = "Image URL must end in .png, .jpg, or .jpeg"
        if(image3 && !isValidFileType(image3)) errors.image3 = "Image URL must end in .png, .jpg, or .jpeg"
        if(image4 && !isValidFileType(image4)) errors.image4 = "Image URL must end in .png, .jpg, or .jpeg"

        setValidationErrors(errors);
    }, [country, address, city, addState, description, title, price, previewImage, image1, image2, image3, image4])
  ...
  return (
    <div className="form-container">
      <form className="create-new-spot" onSubmit={(e) => onSubmit(e)}>
        ...
        <label htmlFor="country">Country{validationErrors.country && <p className="errors">&nbsp;{validationErrors.country}</p>}</label>
        ...
        {validationErrors.address && <p className="errors">&nbsp;{validationErrors.address}</p>}</label>
        
        <input 
            name="street-address" placeholder="Address" 
            value={address}
        ...
        <div className="city-state">
          <div className="city">
            ...
            <input 
                className="location-input" name="city" 
                placeholder="City" 
                value={city}
        ...
        <div className="photos-section">
          ...
          <input 
            ...
            />
            {validationErrors.image1 && <p className="errors">{validationErrors.image1}</p>}
          <input 
                placeholder="Image URL"
                value={image2}
                onChange={(e) => setImage2(e.target.value)}
            />
            {validationErrors.image2 && <p className="errors">{validationErrors.image2}</p>}
          <input 
                placeholder="Image URL"
                value={image3}
                onChange={(e) => setImage3(e.target.value)}
            />
            {validationErrors.image3 && <p className="errors">{validationErrors.image3}</p>}
          <input 
                placeholder="Image URL" 
                value={image4}
                onChange={(e) => setImage4(e.target.value)}
            />
            {validationErrors.image4 && <p className="errors">{validationErrors.image4}</p>}
        </div>
        <button 
        type="submit"
